...
## 当前状态：一期两个能力均已打通 + Web UI

- **CLI**：`harness chat`，终端对话式 Agent。
- **Web UI**：`harness-web`（`python -m harness.web`），浏览器聊天界面（单 HTML 页，无构建步骤），适配显示 nanobot WebUI 风格。
...
CLAUDE.md 要求"基于 nanobot / auto_eval_agent 源码适配"，最高优先级是"最小功能化"。实测依赖闭包过大（nanobot `AgentRunner` 64 文件/2.4 万行、provider 层 5.5k 行；auto_eval_agent 约 50 模块含批跑/集成/元评测/Web 台）。**决策：按两者同构架构自研最小核心**（Provider / Tool / ToolRegistry / AgentLoop + rubric judge），约 1000 行。Web UI 采用 FastAPI + 单 HTML 页（无 React 构建）。两个只读参考副本保留在 `references/` 供对照。
...
| `web.py` | FastAPI Web 服务 + Chat API + 单 HTML 聊天 UI（inline CSS/JS） |
